...
from core.models import Lesson, Syllabus, Module, UserProgress, LessonContent, ConversationHistory
...
def lesson_detail(
    ...
    if request.method == 'POST':
        ...
        logger.warning("Received standard POST on lesson_detail view for lesson %s:%s:%s. Redirecting.", syllabus_id, module_index, lesson_index)
    ...
    try:
        ...
        lesson = get_object_or_404(
            Lesson, module=module, lesson_index=lesson_index
        )
        ...
        if lesson_content:
            if isinstance(lesson_content.content, dict):
                ...
                logger.warning(
                    ...
                    lesson_content.pk, type(lesson_content.content), str(lesson_content.content)[:200] + "..."
                )
                # exposition_content_value remains None
        else:
            # Log that content needs to be generated asynchronously
            logger.info("Lesson content for lesson %s does not exist yet. Will be generated asynchronously.", lesson.pk)
            # exposition_content_value remains None
        ...
        logger.warning(
            ...
            syllabus_id, module_index, lesson_index, # Keep args for context
            exc_info=True
        )
        ...
        logger.error(
            ...
            user.username, syllabus_id, module_index, lesson_index, e,
            exc_info=True
        )
...
def handle_lesson_interaction(
    ...
    try:
        ...
        if progress is None:
             logger.error("Could not retrieve UserProgress for interaction. User %s, Lesson %s", user.username, lesson.pk)
             return JsonResponse({'status': 'error', 'message': 'Could not load user progress.'}, status=500)
        ...
        try:
            ...
            logger.warning("Received invalid JSON in AJAX request from user %s for lesson %s.", user.username, lesson.pk)
            error_message = "Invalid JSON format."
            status_code = 400

        if not error_message and not user_message_content:
            logger.warning("Received empty message from user %s for lesson %s.", user.username, lesson.pk)
        ...
        if not error_message:
            logger.info(
                ...
                submission_type, user.username, lesson.pk
            )
            ...
            if service_response and isinstance(service_response, dict):
                logger.info("Interaction handled successfully for user %s, lesson %s.", user.username, lesson.pk)
                ...
                logger.error(
                    ...
                    user.username, lesson.pk, service_response
                )
...
def generate_lesson_content_async(
    ...
    logger.info(
        ...
        syllabus_id, module_index, lesson_index, user.username
    )
    ...
    try:
        ...
        if lesson_content and isinstance(lesson_content.content, dict):
            ...
            if isinstance(raw_exposition_value, str) and raw_exposition_value.strip().startswith('{"exposition":'):
                try:
                    logger.warning("Detected potential double encoding in async view for lesson %s. Attempting to parse inner JSON.", lesson.pk)
                    inner_data = json.loads(raw_exposition_value)
                    if isinstance(inner_data, dict):
                        exposition_markdown = inner_data.get('exposition', '') # Get the actual markdown
                except json.JSONDecodeError:
                    logger.error("Failed to parse potentially double-encoded JSON in async view for lesson %s.", lesson.pk, exc_info=True)
            ...
            if exposition_markdown:
                ...
                logger.info("Async content generation successful for lesson %s.", lesson.pk)
                return JsonResponse({
                    'status': 'success',
                    'html_content': mark_safe(html_content) # Ensure it's marked safe
                })
            else:
                logger.error("Generated content for lesson %s is missing 'exposition' key or value.", lesson.pk)
                error_message = "Generated content is invalid."
        else:
            logger.error("Failed to get or generate content for lesson %s via async request.", lesson.pk)
        ...
        return JsonResponse({'status': 'error', 'error': 'Lesson not found.'}, status=404)
    except Exception as e:
        logger.exception(
