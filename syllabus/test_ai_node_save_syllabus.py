...
def existing_user_syllabus(test_user):
    ...
    module = Module.objects.create(
        syllabus=syllabus, module_index=0, title="Existing Save Mod 1"
    )  # pylint: disable=no-member
    Lesson.objects.create(
        module=module, lesson_index=0, title="Existing Save Lsn 1.1", duration=5
    )  # pylint: disable=no-member
        ...
        "modules": [
            {
                ...
                "lessons": [
                    {
                        "title": "Save Lesson 1.1",
                        "summary": "First save",
                        "duration": 10,
                    },
                    {
                        "title": "Save Lesson 1.2",
                        "summary": "Second save",
                        "duration": 20,
                    },
                ],
...
def test_save_syllabus_update_existing(existing_user_syllabus):
    ...
    initial_state["existing_syllabus"] = (
        None  # Assume DB search didn't populate this fully
    )
    initial_state["generated_syllabus"] = (
        generated_syllabus_content  # Provide the new content
    )
...
def test_save_syllabus_no_content():
    ...
    initial_state_dict = initialize_state(
        None, topic="No Content Topic", knowledge_level="beginner"
    )
    initial_state = cast(SyllabusState, initial_state_dict)
    ...
    assert (
        "No generated syllabus content found in state" in result_state["error_message"]
    )
...
def test_save_syllabus_invalid_content_format():
    ...
    initial_state_dict = initialize_state(
        None, topic="Bad Format Topic", knowledge_level="beginner"
    )
    initial_state = cast(SyllabusState, initial_state_dict)
    ...
    assert result_state["saved_uid"] is None
    assert (
        "Invalid format for syllabus_to_save: Expected dict, got <class 'str'>"
...
def test_save_syllabus_invalid_user_id(test_user):
    ...
    initial_state_dict = initialize_state(
        None, topic=topic, knowledge_level=level, user_id="nonexistent-user-pk"
    )
    initial_state = cast(SyllabusState, initial_state_dict)
    initial_state["generated_syllabus"] = generated_syllabus_content
    ...
    assert (
        "Invalid User ID format 'nonexistent-user-pk'" in result_state["error_message"]
    )
