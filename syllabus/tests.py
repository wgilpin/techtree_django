...
def test_get_lesson_details_success(syllabus_service, existing_syllabus):
    ...
    module_index = 0
    lesson_index = 1
    result = syllabus_service.get_lesson_details(syllabus_id, module_index, lesson_index)
...
def test_get_lesson_details_syllabus_not_found(syllabus_service):
    ...
    assert f"Lesson 0 not found in module 0, syllabus {non_existent_id}" in str(excinfo.value)
...
def test_get_lesson_details_module_not_found(syllabus_service, existing_syllabus):
    ...
    assert f"Lesson 0 not found in module {non_existent_index}, syllabus {syllabus_id}" in str(excinfo.value)

# --- get_or_generate_syllabus ---

def test_get_or_generate_syllabus_existing(syllabus_service, existing_syllabus, test_user):
...
def test_get_or_generate_syllabus_generate_success(mock_get_ai, syllabus_service, test_user):
    ...
    generated_syllabus_content = { # Content doesn't strictly matter here, just needs to be a dict
        "topic": topic, "level": level, "modules": [{"title": "Gen Mod"}]
    }
    ...
    mock_module.title = "Generated Module 1"; mock_module.summary = "Gen Summary 1"; mock_module.module_index = 0; mock_module.id = 1; mock_module.created_at = timezone.now(); mock_module.updated_at = timezone.now()
    mock_lesson = MagicMock(spec=Lesson)
    mock_lesson.title = "Gen Lesson 1.1"; mock_lesson.summary = "Gen L Summary 1.1"; mock_lesson.duration = 12; mock_lesson.lesson_index = 0; mock_lesson.id = 1
    ...
    with patch('syllabus.services.Syllabus.objects') as mock_manager:
        ...
        def get_side_effect(*args, **kwargs):
            ...
            raise ValueError(f"Unexpected call to Syllabus.objects.get with args={args}, kwargs={kwargs}")

        # Configure the mock manager chain to use the side effect for .get()
        mock_manager.prefetch_related.return_value.select_related.return_value.get.side_effect = get_side_effect

        # --- Call the service method ---
        result = syllabus_service.get_or_generate_syllabus(topic=topic, level=level, user=test_user)
    ...
    get_mock = mock_manager.prefetch_related.return_value.select_related.return_value.get
...
@patch('syllabus.services.SyllabusService._get_syllabus_ai_instance')
def test_get_or_generate_syllabus_generate_fail_no_content(mock_get_ai, syllabus_service, test_user):
    ...
    with pytest.raises(ApplicationError) as excinfo:
        syllabus_service.get_or_generate_syllabus(topic=topic, level=level, user=test_user)
    assert "Failed to generate syllabus content" in str(excinfo.value)


@patch('syllabus.services.SyllabusService._get_syllabus_ai_instance')
def test_get_or_generate_syllabus_generate_fail_no_uid(mock_get_ai, syllabus_service, test_user):
...
@patch('syllabus.services.SyllabusService._get_syllabus_ai_instance')
def test_get_or_generate_syllabus_ai_exception(mock_get_ai, syllabus_service, test_user):
...
def test_get_lesson_details_lesson_not_found(syllabus_service, existing_syllabus):
    ...
    non_existent_index = 99
    with pytest.raises(NotFoundError) as excinfo:
        syllabus_service.get_lesson_details(syllabus_id, module_index, non_existent_index)
    assert f"Lesson {non_existent_index} not found in module {module_index}, syllabus {syllabus_id}" in str(excinfo.value)

    non_existent_index = 99
    with pytest.raises(NotFoundError) as excinfo:
        syllabus_service.get_module_details(syllabus_id, non_existent_index)
    assert f"Module {non_existent_index} not found in syllabus {syllabus_id}" in str(excinfo.value)
...
def test_syllabus_detail_success(mock_get_syllabus, logged_in_client, existing_syllabus):
    """Test syllabus detail view renders correctly."""
    syllabus_id = str(existing_syllabus.syllabus_id)
    # Mock the service call to return formatted data
    mock_get_syllabus.return_value = SyllabusService()._format_syllabus_dict(existing_syllabus)
...
def test_module_detail_success(mock_get_module, logged_in_client, existing_syllabus):
    ...
    mock_get_module.assert_called_once_with(uuid.UUID(syllabus_id), module_index) # Expect UUID

def test_module_detail_unauthenticated(client, existing_syllabus):
    """Test module detail view redirects unauthenticated users."""
    url = reverse("syllabus:module_detail", args=[str(existing_syllabus.syllabus_id), 0])
    response = client.get(url)
    assert response.status_code == 302
    assert '/accounts/login/' in response.url
...
def test_lesson_detail_success(mock_get_lesson, logged_in_client, existing_syllabus):
    ...
    # Mock the service call
    mock_lesson_data = {
        "id": 2, "module_id": 1, "syllabus_id": syllabus_id,
        "lesson_index": lesson_index, "title": "Lesson 1.2",
        "summary": "Summary 1.2", "duration": 15
    }
    ...
    mock_get_lesson.assert_called_once_with(uuid.UUID(syllabus_id), module_index, lesson_index) # Expect UUID

def test_lesson_detail_unauthenticated(client, existing_syllabus):
    """Test lesson detail view redirects unauthenticated users."""
    url = reverse("syllabus:lesson_detail", args=[str(existing_syllabus.syllabus_id), 0, 0])
    response = client.get(url)
    assert response.status_code == 302
    assert '/accounts/login/' in response.url
...
def test_lesson_detail_service_error(mock_get_lesson, logged_in_client):
    ...
    mock_get_lesson.assert_called_once_with(uuid.UUID(syllabus_id), module_index, lesson_index) # Check UUID passed
...
def test_generate_syllabus_missing_id_response(mock_generate, logged_in_client):
    ...
    url = reverse("syllabus:generate")
    response = logged_in_client.post(url, {"topic": "Incomplete Topic", "level": "beginner"})
