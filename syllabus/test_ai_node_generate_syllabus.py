...
from syllabus.ai.nodes import initialize_state, generate_syllabus
...
def test_generate_syllabus_success(mock_call_retry):
    ...
    mock_response_content = {
        ...
        "modules": [
            {"title": "Generated Module 1", "summary": "Summary 1", "lessons": [{"title": "Lesson 1.1", "summary": "S1", "duration": 10}]}
        ]
    }
    ...
    assert result_state["generated_syllabus"]["modules"] == mock_response_content["modules"]
    assert len(result_state["generated_syllabus"]["modules"]) == 1 # Re-assert length for clarity
    assert result_state["generated_syllabus"]["modules"][0]["title"] == "Generated Module 1"
    ...
    assert call_args[0] == mock_llm.generate_content # Check it targeted the generate_content method
...
def test_generate_syllabus_invalid_json(mock_call_retry):
    ...
    assert result_state["generated_syllabus"] is not None
    assert isinstance(result_state["generated_syllabus"], dict)
    assert result_state["generated_syllabus"]["topic"] == topic # Fallback uses initial topic
    assert result_state["generated_syllabus"]["level"] == level # Fallback uses initial level
    # Assert fallback structure (has 2 default modules)
    assert len(result_state["generated_syllabus"]["modules"]) == 2
    assert result_state["generated_syllabus"]["modules"][0]["title"] == f"Introduction to {topic}"
    assert "LLM call failed or returned empty/invalid JSON" in result_state["error_message"]
...
def test_generate_syllabus_llm_failure(mock_call_retry):
    ...
    assert isinstance(result_state["generated_syllabus"], dict)
    assert result_state["generated_syllabus"]["topic"] == topic
    assert result_state["generated_syllabus"]["level"] == level
    # Assert fallback structure (has 2 default modules)
    assert len(result_state["generated_syllabus"]["modules"]) == 2
    assert result_state["generated_syllabus"]["modules"][0]["title"] == f"Introduction to {topic}"
...
def test_generate_syllabus_no_llm():
    ...
    assert result_state["generated_syllabus"]["modules"] == [{"title": "Generation Failed", "summary": "Fallback due to error.", "lessons": []}]
