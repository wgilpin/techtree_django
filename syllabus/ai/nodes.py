...
def search_database(state: SyllabusState) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warning(f"User with ID {user_id} not found. Searching for master syllabus.")
    ...
    try:
        syllabus_obj = Syllabus.objects.prefetch_related( # pylint: disable=no-member
        ...
        syllabus_data = {
            ...
            "uid": str(syllabus_obj.syllabus_id), # Map uid to syllabus_id for compatibility
            "topic": syllabus_obj.topic,
            "level": syllabus_obj.level,
            "user_entered_topic": syllabus_obj.user_entered_topic or state.get("user_entered_topic", topic),
            "user_id": str(syllabus_obj.user.pk) if isinstance(syllabus_obj.user, User) else None, # Added isinstance check for Mypy
            ...
            "duration": syllabus_obj.duration if hasattr(syllabus_obj, 'duration') else "N/A", # Placeholder if not on model
            "learning_objectives": syllabus_obj.learning_objectives if hasattr(syllabus_obj, 'learning_objectives') else [], # Placeholder if not on model
        }
        ...
        return {"existing_syllabus": None, "uid": None, "error_message": None} # Return uid: None when not found, no error message here
    except Exception as e:
        error_msg = f"DB search error: {e}"
        logger.error(f"Error searching database for syllabus: {e}", exc_info=True)
        return {"existing_syllabus": None, "uid": None, "error_message": error_msg} # Return None on error and message


def search_internet(
    state: SyllabusState, tavily_client: Optional[TavilyClient]
) -> Dict[str, Any]: # Return type includes error message
    ...
    if not tavily_client:
        ...
        return {"search_results": [], "search_queries": [], "error_message": "Tavily client not configured."}
    ...
    for query, params in queries:
        ...
        try:
            ...
            # Pass the tavily search method to call_with_retry
            search_response = call_with_retry(tavily_client.search, query=query, search_depth="advanced", **params) # type: ignore
            # Process the response dictionary
            content = [
                r.get("content", "")
                for r in search_response.get("results", []) # Extract from 'results' key
                if r.get("content")
            ]
            ...
            search_results.append(error_msg) # Add error to results for context
            return {"search_results": search_results, "search_queries": all_queries, "error_message": error_msg} # Return immediately on error
        except Exception as e:
            logger.error(
                f"Unexpected error during Tavily search for query '{query}': {e}",
                exc_info=True,
            )
            error_msg = f"Unexpected error during web search for query '{query}': {str(e)}"
            search_results.append(error_msg)
            return {"search_results": search_results, "search_queries": all_queries, "error_message": error_msg} # Return immediately on error

    logger.info(f"Total search results gathered: {len(search_results)}")
    # Return success state including queries
    return {"search_results": search_results, "search_queries": all_queries, "error_message": None}
...
def _validate_syllabus_structure(
    ...
    for i, module in enumerate(syllabus["modules"]):
        ...
        if not isinstance(module.get("lessons"), list): # Check it's a list, even if empty
             logger.error(
                 f"Error: {context} JSON module {i} 'lessons' must be a list."
             )
             return False
...
def generate_syllabus(
    ...
    if not llm_model:
        ...
        return {
            "generated_syllabus": {
                ...
                "modules": [{"title": "Generation Failed", "summary": "Fallback due to error.", "lessons": []}],
            },
    ...
    try:
        ...
        response_text = f"LLM Error: {e}" # Store error in response_text for fallback message
    ...
    if syllabus and _validate_syllabus_structure(syllabus, "Generated"):
        return {"generated_syllabus": syllabus, "error_message": None, "error_generating": False}
    else:
        logger.warning(
            ...
            "modules": [
                {
                    ...
                    "lessons": [
                        {"title": "What is " + topic + "?", "summary": "Definition and scope.", "duration": 10},
                        {"title": "Core Terminology", "summary": "Key terms explained.", "duration": 15},
                        {"title": "Real-world Examples", "summary": "Applications in practice.", "duration": 10},
                    ],
                },
                {
                    # "unit": 2,
                    ...
                    "lessons": [
                        {"title": "Principle A", "summary": "Explanation of Principle A.", "duration": 20},
                        {"title": "Principle B", "summary": "Explanation of Principle B.", "duration": 15},
                        {"title": "How Principles Interact", "summary": "Combining principles.", "duration": 10},
                    ],
                },
            ],
        }
        return {
            ...
            "error_message": f"LLM call failed or returned empty/invalid JSON: {response_text[:200]}..." if response_text else "LLM call failed or returned empty.",
            "error_generating": True,
        }
...
def update_syllabus(
    ...
    if not llm_model:
        ...
        return {
            ...
            "generated_syllabus": state.get("generated_syllabus") or state.get("existing_syllabus"),
...
def save_syllabus(state: SyllabusState) -> Dict[str, Any]:
    ...
    # db_service parameter removed
    syllabus_to_save = state.get("generated_syllabus") # Prioritize generated for saving/updating
    if not syllabus_to_save:
        ...
        return {"syllabus_saved": False, "saved_uid": None, "error_message": "No generated syllabus content found in state."}
    ...
    try:
        ...
        if uid_to_update:
            logger.info(f"Attempting to update existing syllabus with UID: {uid_to_update}")
            syllabus_instance, created = Syllabus.objects.update_or_create( # pylint: disable=no-member
        ...
        for module_index, module_data in enumerate(modules_data):
            if not isinstance(module_data, dict):
                logger.warning(f"Skipping invalid module data (not a dict) at index {module_index}")
                continue
            ...
            if not isinstance(lessons_data, list):
                logger.warning(f"Skipping invalid lessons data (not a list) in module '{module_title}'")
                continue

            for lesson_index, lesson_data in enumerate(lessons_data):
                if not isinstance(lesson_data, dict):
                    logger.warning(f"Skipping invalid lesson data (not a dict) at index {lesson_index} in module '{module_title}'")
                    continue
        ...
        return {"syllabus_saved": False, "saved_uid": None, "error_message": f"DB save error: {e}"}
