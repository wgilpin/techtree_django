...
async def test_update_syllabus_success(mock_call_retry):
    ...
    initial_state["generated_syllabus"] = (
        initial_syllabus_content  # Assume this was generated
    )
    ...
    updated_syllabus_content = {
        ...
        "modules": [
            {"title": "Original Module", "lessons": []},
            {
                ...
                "summary": "Added based on feedback",
                "lessons": [],
            },
        ],
    }
    ...
    result_state_dict = await update_syllabus(
        initial_state, feedback, llm_model=mock_llm
    )
    ...
    assert (
        len(result_state["generated_syllabus"]["modules"]) == 2
    )  # Re-assert length for clarity
    assert (
        result_state["generated_syllabus"]["modules"][1]["title"] == "Advanced Concepts"
    )
    ...
    assert call_args[0] == mock_llm.generate_content_async  # Check method called
...
@pytest.mark.asyncio
@patch("syllabus.ai.nodes.call_with_retry", new_callable=AsyncMock)
async def test_update_syllabus_llm_failure(mock_call_retry):
...
@pytest.mark.asyncio
async def test_update_syllabus_no_llm():
    ...
    result_state_dict = await update_syllabus(initial_state, feedback, llm_model=None)
    result_state = cast(SyllabusState, result_state_dict)
...
@pytest.mark.asyncio
async def test_update_syllabus_no_initial_syllabus():
    ...
    assert (
        "No syllabus content found in state to update" in result_state["error_message"]
    )
    assert result_state["error_generating"] is True
