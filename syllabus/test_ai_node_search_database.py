...
def existing_user_syllabus(test_user):
    ...
    module = Module.objects.create(syllabus=syllabus, module_index=0, title="User Mod 1") # pylint: disable=no-member
    Lesson.objects.create(module=module, lesson_index=0, title="User Lsn 1.1", duration=5) # pylint: disable=no-member
...
def existing_master_syllabus():
    ...
    module = Module.objects.create(syllabus=syllabus, module_index=0, title="Master Mod 1") # pylint: disable=no-member
    Lesson.objects.create(module=module, lesson_index=0, title="Master Lsn 1.1", duration=10) # pylint: disable=no-member
    # Refresh to ensure relations are loaded
    return Syllabus.objects.prefetch_related("modules__lessons").get(pk=syllabus.pk) # pylint: disable=no-member
...
def test_search_database_finds_user_syllabus(test_user, existing_user_syllabus):
    ...
    assert result_state["existing_syllabus"]["level"] == "intermediate"
    ...
    assert result_state["existing_syllabus"]["modules"][0]["lessons"][0]["title"] == "User Lsn 1.1"
...
def test_search_database_finds_master_syllabus(existing_master_syllabus):
    ...
    assert result_state["existing_syllabus"]["level"] == "expert"
