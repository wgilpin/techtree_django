...
def _get_llm(model_key="FAST_MODEL", temperature=0.2) -> Optional[ChatGoogleGenerativeAI]:
    ...
    try:
        ...
        logger.error("Failed to initialize ChatGoogleGenerativeAI: %s", e, exc_info=True)
        return None
...
class TechTreeAI:
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            logger.error("Failed to initialize TavilySearchResults: %s", e, exc_info=True)
            raise ValueError("Search tool could not be initialized.") from e

        # LangGraph setup (simplified for direct method calls)
        # In a full LangGraph implementation, nodes would be defined here
        # and added to a graph. For now, methods represent node logic.
        self.graph = self._build_graph() # Keep graph building logic if needed later

    def _build_graph(self):
         # Placeholder for potential future LangGraph implementation
         workflow = StateGraph(AgentState)
         # Add nodes and edges here if using full LangGraph
         # workflow.add_node(...)
         # workflow.add_edge(...)
         # workflow.set_entry_point(...)
         # workflow.add_conditional_edges(...)
         # return workflow.compile()
         return None # Return None as we are calling methods directly for now
    ...
    async def perform_internet_search(self, state: AgentState) -> Dict[str, Any]:
        ...
        if not search_queries:
             ...
             logger.warning("No search queries found in state for topic: %s", state['topic'])
             query = state['topic'] # Fallback to topic itself
        else:
             query = search_queries[-1] # Use the latest query

        try:
            search_results = await call_with_retry(self.search_tool.invoke, {"query": query})
            ...
            logger.error("Internet search failed for query '%s': %s", query, e, exc_info=True)
            return {"error_message": f"Search failed: {e}", "search_completed": True} # Mark search as completed even on error
    ...
    async def generate_question(self, state: AgentState) -> Dict[str, Any]:
        ...
        if not self.llm: return {"error_message": "LLM not available"}
        ...
        prompt_input = {
            ...
            "difficulty": state.get("current_target_difficulty", settings.ONBOARDING_DEFAULT_DIFFICULTY),
        ...
        try:
            ...
            if not isinstance(question_data, dict) or not question_data.get("question"):
                 raise ValueError("LLM response did not contain a valid question structure.")

            logger.info("Successfully generated question for topic: %s", state['topic'])

            # Update state
            new_questions = state.get("questions_asked", []) + [question_data]
            new_difficulties = state.get("question_difficulties", []) + [question_data.get("difficulty", settings.ONBOARDING_DEFAULT_DIFFICULTY)]

            return {
                "questions_asked": new_questions,
                "question_difficulties": new_difficulties,
                "current_question": question_data["question"],
                "current_question_difficulty": question_data.get("difficulty", settings.ONBOARDING_DEFAULT_DIFFICULTY),
                "error_message": None, # Clear previous errors
            }
        except json.JSONDecodeError as e:
             logger.error("Failed to parse JSON from question generation response: %s\nContent: %s", e, content, exc_info=True)
    ...
    async def evaluate_answer(self, state: AgentState, answer: str) -> Dict[str, Any]:
        ...
        prompt_input = {
            "topic": state.get("topic", "Unknown Topic"),
            "level": state.get("knowledge_level", "beginner"),
            "question": json.dumps(current_question_data), # Pass the full question data
            "user_answer": answer,
        }
        ...
        try:
            ...
            if not isinstance(evaluation_data, dict) or "score" not in evaluation_data or "feedback" not in evaluation_data:
                 raise ValueError("LLM response did not contain valid evaluation structure (score, feedback).")

            logger.info("Successfully evaluated answer for topic: %s", state['topic'])

            # Update state
            new_answers = state.get("answers", []) + [{"answer": answer, "feedback": evaluation_data["feedback"]}]
            new_evaluations = state.get("answer_evaluations", []) + [evaluation_data["score"]]
            ...
            current_difficulty = state.get("current_question_difficulty", settings.ONBOARDING_DEFAULT_DIFFICULTY)
            ...
            if evaluation_data["score"] < 0.5: # Threshold for wrong
                ...
                consecutive_wrong = 0
                if current_difficulty >= settings.ONBOARDING_HARD_DIFFICULTY_THRESHOLD and evaluation_data["score"] >= 0.7: # Threshold for hard correct/partial
            ...
            return {
             ...
             logger.error("Failed to parse JSON from evaluation response: %s\nContent: %s", e, content, exc_info=True)
             return {"error_message": f"AI response parsing error: {e}"}
        except Exception as e:
    ...
    def calculate_final_assessment(self, state: AgentState) -> Dict[str, Any]:
        ...
        final_assessment_data = {
            ...
            "responses": state.get("answers"), # Assuming 'answers' holds response dicts
        }
