    ...
    try:
        ...
        if hasattr(settings, 'BASE_DIR'):
            ...
            # Fallback if BASE_DIR isn't defined early enough (less likely with Django)
             ...
             print(f"pytest_configure: Warning - settings.BASE_DIR not found, guessing log path: {log_file_path}")
            ...
            if isinstance(handler, TimedRotatingFileHandler):
                ...
                try:
                    ...
                    if handler_path == target_path:
                     ...
                     print(f"pytest_configure: Error comparing handler path '{getattr(handler, 'baseFilename', 'N/A')}' with target '{log_file_path}': {path_e}")


        if handler_to_remove:
            print(f"pytest_configure: Found file handler for {log_file_path}. Removing it.")
            ...
            print(f"pytest_configure: Did not find specific file handler for {log_file_path} on root logger.")

    except Exception as e:
        # Use pytest's warning system for better visibility during test setup
        config.warn("CONF_LOGGING_ERROR", f"Error configuring logging in pytest_configure: {e}")