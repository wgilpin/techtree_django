...
def process_ai_task(task_id):
    ...
    try:
        ...
        logger.info(f"Processing task {task_id} of type {task.task_type}, attempt {task.attempt_count}")
        
        # Route to appropriate processor based on task type
        if task.task_type == AITask.TaskType.SYLLABUS_GENERATION:
            from taskqueue.processors.syllabus_processor import process_syllabus_generation
...
def get_queue_metrics():
    ...
    processing_count = AITask.objects.filter(status=AITask.TaskStatus.PROCESSING).count()
