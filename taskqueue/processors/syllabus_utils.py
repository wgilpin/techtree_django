...
def process_syllabus_generation(task):
    ...
    if not topic or not knowledge_level:
        raise ValueError(
            "Missing required parameters: topic and knowledge_level are required"
        )
    ...
    try:
        ...
        syllabus = result_state.get("generated_syllabus") or result_state.get(
            "existing_syllabus"
        )

        if not syllabus:
            raise ValueError(
                "Syllabus generation completed but no syllabus was found in the result state"
            )
        ...
        if syllabus_id:
            try:
                ...
                logger.error(
                    "Error triggering first lesson content generation: %s",
                    e,
                    exc_info=True,
                )
